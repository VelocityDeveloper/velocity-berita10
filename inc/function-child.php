...
function velocitychild_theme_setup() {	
	...
	if (class_exists('Kirki')) :
		...
		$fieldiklan = [
			...
			'iklan_home'  => [
				'label'			=> 'Iklan Halaman Depan',
				'description'	=> 'Ukuran gambar 140x600',
			],
			'iklan_single'  => [
		...
		$fieldposts = [
			...
			'posts_left_home'  => [
				'label'		=> 'Posts Left Home',
				'section'	=> 'section_homeberita',
			],
			'posts_middle_home1'  => [
				'label'		=> 'Posts Middle Home 1',
				'section'	=> 'section_homeberita',
			],
			'posts_middle_home2'  => [
				'label'		=> 'Posts Middle Home 2',
				'section'	=> 'section_homeberita',
			],
			'posts_footer_home1'  => [
				'label'		=> 'Posts Footer Home 1',
				'section'	=> 'section_homeberita',
			],
			'posts_footer_home2'  => [
				'label'		=> 'Posts Footer Home 2',
				'section'	=> 'section_homeberita',
			],
			'posts_footer_home3'  => [
				'label'		=> 'Posts Footer Home 3',
				'section'	=> 'section_homeberita',
			],
		];
...
function velocity_post_carousel($cat_id = null) {
    ...
    if ($the_query->have_posts()) {
        ...
        while ($the_query->have_posts()) {
			...
			echo '<div class="row">';
				echo '<div class="col-4 col-md-5 velocity-post-thumbnail">';
					echo do_shortcode('[resize-thumbnail width="300" height="220" linked="true" class="w-100"]');
				echo '</div>';
				echo '<div class="col-8 col-md-7 ps-0 velocity-post-title">';
					echo '<a class="secondary-font text-dark" href="'.get_the_permalink().'">'.get_the_title().'</a>';
					echo '<small class="text-color-theme d-block mt-1">'.get_the_date().'</small>';
				echo '</div>';
			echo '</div>';			
			echo '</div>';			
        }
...
function velocity_post_list($post_id = null, $date = true) {
    global $post;
    if (empty($post_id)){
        $post_id = $post->ID;
    }
	echo '<div class="mb-1 pb-1 border-bottom row mx-0">';
	echo '<div class="col-1 px-0 text-center">';
		echo '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-file-earmark-text text-secondary" viewBox="0 0 16 16">
	...
	echo '<div class="col-11 px-0">';
		if($date){
			echo '<small class="d-block text-color-theme mb-0">'.get_the_date('',$post_id).'</small>';
		}
		echo '<a class="secondary-font text-color-theme fw-bold" href="'.get_the_permalink($post_id).'">'.get_the_title($post_id).'</b></a>';
	echo '</div>';
	echo '</div>';
}
...
function velocity_post_loop($post_id = null, $width = 300, $height = 250) {
    global $post;
    if (empty($post_id)){
        $post_id = $post->ID;
    }
...
// Menampilkan post grid 
function velocity_post_grid($post_id = null) {
    global $post;
    if (empty($post_id)){
        $post_id = $post->ID;
    }
	echo '<div class="mb-2 col-md-3 col-6 px-1">';
		echo '<div class="mb-1">';
			echo do_shortcode('[resize-thumbnail width="300" height="200" linked="true" class="w-100" post_id="'.$post_id.'"]');
		echo '</div>';
		echo '<small class="d-block text-color-theme mb-1">';
			echo get_the_date('',$post_id);
		echo '</small>';
...
function velocity_categories(){
	...
	foreach ($parent_categories as $category) {
		if ($category->slug !== 'uncategorized') {
			echo '<li class="list-group-item px-0"><a class="text-color-theme" href="'.get_category_link($category->term_id).'">'.$category->name.'</a></li>';
		}
	}
	echo '</ul>';
}
